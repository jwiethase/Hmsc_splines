...
constructGradient_JHW <- function (hM, focalVariables, ngrid = 25, coordinates = list()) {
   ...
   expr_str <- paste0("smoothCon(s(", base_variable, ", bs = 'tp', k = 3, fx = FALSE), data = XDataNew, absorb.cons = TRUE)[[1]]$X")
   spline_bases <- eval(parse(text = expr_str))
   
   XDataNew[[paste0(base_variable, "_spline1")]] = spline_bases[, 1]
   XDataNew[[paste0(base_variable, "_spline2")]] = spline_bases[, 2]
                              ...
                              showPosteriorSupport = TRUE, back_transform_Y = NULL, unscale_df = NULL, ...) 
{
   ...
   if (showData) {
      ...
      pl <- pl + 
         ...
         ylim(min(pY,plot_data$lo),max(pY,plot_data$hi))
   }
