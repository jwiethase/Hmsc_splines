...
for(mod in model_choices){
      ...
      png(filename = paste0('figures/gradient_plot_SE_', mod, '.png'), width = 25, height = length(gradient_plot_list)*5, units = "cm", res = 300)
      do.call(grid.arrange, c(gradient_plot_list, ncol = 2))
      dev.off()
}
